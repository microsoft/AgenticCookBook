...
class LLamaIndexConversableAgent(ConversableAgent):
    def __init__(        self,
        ...
        **kwargs,
        ...
        self.replace_reply_func(
            ...
            LLamaIndexConversableAgent.generate_reply)
        
        self.replace_reply_func(
            ConversableAgent.a_generate_oai_reply,
            LLamaIndexConversableAgent.a_generate_reply,
        )
    ...
    def generate_reply(
        ...
        user_message, history = self._extract_message_and_history(messages)
    ...
    async def a_generate_reply(
        ...
        """Generate a reply using autogen.oai."""
        user_message, history = self._extract_message_and_history(messages, sender)
    ...
    def _extract_message_and_history(self, messages: Optional[List[Dict]] = None,sender: Optional[Agent] = None) -> Tuple[str, List[ChatMessage]]:
        ...
        if not messages:
            return "", []
        message = content_str(messages[-1])

        history = messages[:-1]
        history_messages : List[ChatMessage] = []
        for history_message in history:
            content = content_str(history_message)
            role = history_message.get("role", None)
            if role:
